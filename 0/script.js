...
function loadSavedTheme() {
    ...
    if (savedTheme && ['win2k', 'winxp', 'vista', 'win7', 'win81', 'win10', 'macos9', 'macosx', 'ribbon'].includes(savedTheme)) {
        setTheme(savedTheme);
    } else {
        setTheme('win2k');
    }
...
function updateGradeColor(inputElement) {
    ...
    inputElement.classList.remove('grade-excellent', 'grade-good', 'grade-poor', 'grade-absent');
    
    if (value === 'Н' || value === 'Ж' || value === 'н' || value === 'ж') {
        inputElement.classList.add('grade-absent');
    } else {
        const num = parseInt(value);
        if (!isNaN(num)) {
            if (num >= 80) inputElement.classList.add('grade-excellent');
            else if (num >= 60) inputElement.classList.add('grade-good');
            else if (num >= 0) inputElement.classList.add('grade-poor');
        }
    }
}

function applyAllGradeColors() {
...
function setActiveCell(inputElement) {
...
function renderTabsContent() {
    ...
    for (let r = 1; r <= activeROCount; r++) {
        ...
        for (let sIdx = 0; sIdx < students.length; sIdx++) {
            ...
            const savedGrade = (gradesData[sIdx] && gradesData[sIdx][r]) ? gradesData[sIdx][r] : {};
                ...
                paneHtml += `<td>
                                <div class="grade-cell">
                                    <input type="text" class="score-input" 
                                           data-s="${sIdx}" data-r="${r}" data-c="${c}" 
                                           value="${val === '' ? '' : val}" 
                                           oninput="saveGrade(${sIdx}, ${r}, ${c}, this.value, this)" 
                                           onkeydown="handleGradeKeydown(event, ${sIdx}, ${r}, ${c}, ${colCount})" 
                                           onclick="setActiveCell(this)"
                                           placeholder="0-100 или ${t('absentMark')}">
    ...
    setTimeout(() => applyAllGradeColors(), 50);
}
...
function saveGrade(sIdx, roIndex, colIndex, value, inputElement) {
    ...
    if (inputElement) {
        updateGradeColor(inputElement);
        setActiveCell(inputElement);
    }
